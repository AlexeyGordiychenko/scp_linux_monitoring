...
color_echo "${params[@]}" "$(echo $mem | awk -v fmt="%.3f" -v div=1000 '{printf "RAM_TOTAL = " fmt " GB", $2/div}')"
color_echo "${params[@]}" "$(echo $mem | awk -v fmt="%.3f" -v div=1000 '{printf "RAM_USED = " fmt " GB", $3/div}')"
color_echo "${params[@]}" "$(echo $mem | awk -v fmt="%.3f" -v div=1000 '{printf "RAM_FREE = " fmt " GB", $4/div}')"
color_echo "${params[@]}" "$(echo $space | awk -v fmt="%.2f" -v div=1000 '{printf "SPACE_ROOT = " fmt " MB", $2/div}')"
color_echo "${params[@]}" "$(echo $space | awk -v fmt="%.2f" -v div=1000 '{printf "SPACE_ROOT_USED = " fmt " MB", $3/div}')"
color_echo "${params[@]}" "$(echo $space | awk -v fmt="%.2f" -v div=1000 '{printf "SPACE_ROOT_FREE = " fmt " MB", $4/div}')"
